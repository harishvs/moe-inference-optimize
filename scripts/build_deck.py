...
def slide_code_overview(prs: Presentation) -> None:
    ...
    set_notes(s,
        ...
        "link. Two columns: what the pipeline does on the left, what makes "
        "the methodology honest on the right.\n\n"
        "Key talking points:\n\n"
        "Reproducibility: every number on every slide came from running "
        "one of these scripts. The deck itself reads JSON from results/ "
        "via talk/numbers.py — re-running the benchmark refreshes the "
        "slides without me touching them. That's the discipline that "
        "lets the next person re-baseline when the model or hardware "
        "changes.\n\n"
        "Engine-agnostic harness: scripts/harness/ wraps the vLLM server "
        "lifecycle behind an OpenAI-compatible HTTP client. Swap the "
        "server launcher and the same benchmarks run against TRT-LLM, "
        "SGLang, or anything else with an OpenAI endpoint. That's "
        "deliberate — when the next model arrives, the harness doesn't "
        "have to be rewritten.\n\n"
        "Seed pinning: every benchmark uses seed=42 for prompt generation. "
        "Variance across runs is under 1 ms — that's how we can trust "
        "small deltas as signal, not noise.\n\n"
        "If asked: code is on the next slide, scan the QR."
    )
